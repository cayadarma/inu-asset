  ...
  return (
    <div className="flex flex-col gap-8 pb-10 font-poppins">
      ...
      <div>
        ...
        <p className="text-[#475569] text-sm">Hasilkan laporan komprehensif aset and pemeliharaan secara instan</p>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-5 gap-8 items-start">
        
        {/* KIRI: Konfigurasi Ekspor Laporan */}
        <div className="xl:col-span-2 bg-white p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-8">
          <h3 className="font-bold text-[#0F172A] text-lg">Konfigurasi Ekspor Laporan</h3>
          
          <div className="flex flex-col gap-6">
            {/* Dropdown Tipe Laporan */}
            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-bold text-[#475569] uppercase tracking-wider">Tipe Laporan</label>
              <div className="relative">
                <select 
                  value={reportType}
                  onChange={(e) => setReportType(e.target.value)}
                  className="w-full appearance-none px-4 py-3 border border-gray-200 rounded-xl bg-[#F8FAFC] text-sm text-[#0F172A] outline-none focus:border-primary cursor-pointer font-semibold"
                >
            ...
            {/* Dropdown Lokasi */}
            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-bold text-[#475569] uppercase tracking-wider">Lokasi</label>
              <div className="relative">
                <select 
                  value={location}
                  onChange={(e) => setLocation(e.target.value)}
                  className="w-full appearance-none px-4 py-3 border border-gray-200 rounded-xl bg-white text-sm text-[#0F172A] outline-none focus:border-primary cursor-pointer font-semibold"
                >
            ...
            {/* Dropdown Periode */}
            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-bold text-[#475569] uppercase tracking-wider">Periode</label>
              <div className="relative">
                <select 
                  value={period}
            ...
            <div className="flex flex-col gap-3">
              <label className="text-[13px] font-bold text-[#475569] uppercase tracking-wider">Format Laporan</label>
              <div className="flex gap-6">
                ...
                >
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center p-1 transition-all ${format === 'pdf' ? 'border-[#0D9488]' : 'border-gray-200'}`}>
                    {format === 'pdf' && <div className="w-full h-full bg-[#0D9488] rounded-full"></div>}
                  </div>
                  <span className={`text-sm ${format === 'pdf' ? 'font-bold text-[#0F172A]' : 'font-medium text-[#94A3B8]'}`}>PDF Document (.pdf)</span>
                </button>
                ...
                <button 
                  ...
                  className="flex items-center gap-3 group cursor-pointer"
                >
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center p-1 transition-all ${format === 'excel' ? 'border-[#0D9488]' : 'border-gray-200'}`}>
                    {format === 'excel' && <div className="w-full h-full bg-[#0D9488] rounded-full"></div>}
                  </div>
                  <span className={`text-sm ${format === 'excel' ? 'font-bold text-[#0F172A]' : 'font-medium text-[#94A3B8]'}`}>Excel Sheet (.xlsx)</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 pt-4">
            ...
            <Link 
              ...
              className="w-full bg-white text-[#475569] border border-gray-200 py-4 rounded-xl font-bold text-sm hover:bg-gray-50 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
            >
        ...
        <div className="xl:col-span-3 bg-white p-8 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-6">
          <h3 className="font-bold text-[#0F172A] text-lg">Laporan Terbaru Yang Diunduh</h3>
                ...
                {recentReports.map((report, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors group">
                    <td className="px-6 py-5">
                      <p className="font-bold text-[#0F172A] truncate max-w-[250px]">{report.name}</p>
