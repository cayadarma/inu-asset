...
export default function BukuSakitPage() {
  ...
  // Terjemahkan nama lokasi (teks bebas dari DB) sekaligus dalam satu batch
  const dynamicMap = useDynamicTextMap(locations.map((l) => l.name));
          ...
          >
            <div className="flex items-start gap-6">
              ...
              <div className="flex flex-col gap-3">
                <h3 className="text-lg font-bold text-[#0F172A] dark:text-[#F8FAFC] uppercase">{dt(loc.name)}</h3>
                <div className="flex flex-wrap gap-x-8 gap-y-2">
                   <div className="flex flex-col gap-1">
                     <span className="px-3 py-0.5 bg-[#D1FAE5] text-[#065F46] text-[10px] font-bold rounded-full">{t("bukuSakit.list.asetBeroperasi", { count: loc.active })}</span>
                     <span className="px-3 py-0.5 bg-[#FEF3C7] text-[#EF4444] text-[10px] font-bold rounded-full">{t("bukuSakit.list.asetPerbaikan", { count: loc.repair })}</span>
                     <span className="px-3 py-0.5 bg-[#EDE9FE] text-[#6D28D9] text-[10px] font-bold rounded-full">{t("bukuSakit.list.asetIdle", { count: loc.idle })}</span>
                   </div>
                   <div className="flex flex-col gap-1">
                     <span className="px-3 py-0.5 bg-[#FFF7D6] text-[#E28E00] text-[10px] font-bold rounded-full">{t("bukuSakit.list.asetPemeliharaan", { count: loc.maintenance })}</span>
                     <span className="px-3 py-0.5 bg-[#FEE2E2] text-[#991B1B] text-[10px] font-bold rounded-full">{t("bukuSakit.list.asetRusak", { count: loc.broken })}</span>
                     <span className="px-3 py-0.5 bg-[#E2E8F0] text-[#475569] text-[10px] font-bold rounded-full">{t("bukuSakit.list.asetNonAktif", { count: loc.nonaktif })}</span>
                   </div>
                </div>
              </div>
            </div>
