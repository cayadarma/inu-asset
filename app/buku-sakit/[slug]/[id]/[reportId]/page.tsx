...
export default function FinalReportDetailPage({ params }: { params: Promise<{ slug: string, id: string, reportId: string }> }) {
  ...
  // Terjemahkan teks bebas dari DB (judul, pelapor, deskripsi, nama/tipe aset, nama lokasi) sekaligus
  const dynamicMap = useDynamicTextMap([
    ...
    report?.assets?.name,
    report?.assets?.type,
    report?.assets?.locations?.name,
  ]);
  const dt = (text: string | null | undefined) => (text ? dynamicMap.get(text.trim()) ?? text : text);

  if (isLoading) return <div className="p-20 text-center font-bold dark:text-white font-poppins">{t("bukuSakit.report.memuatDetail")}</div>;
  if (!report) return <div className="p-20 text-center text-red-500 font-bold">{t("bukuSakit.report.laporanTidakDitemukan")}</div>;
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1100px] mx-auto pb-10 font-poppins text-left">
      ...
      >
        ...
        {t("bukuSakit.report.kembaliRiwayat")}
      </Link>
      ...
      <div className="bg-white dark:bg-[#1E293B] rounded-[32px] border border-gray-100 dark:border-[#334155] shadow-2xl overflow-hidden transition-all duration-300">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3">
              <span className="px-3 py-1 bg-[#EF4444]/10 text-[#EF4444] text-[10px] font-black rounded-full uppercase tracking-tighter border border-[#EF4444]/20">
                {t("bukuSakit.report.laporanKerusakan")}
              </span>
            ...
            <h1 className="text-3xl font-black text-[#0F172A] dark:text-[#F8FAFC] tracking-tight mt-1 capitalize leading-tight">
              {dt(report.issue_title)}
            </h1>
          </div>
          <div className={`${innerBoxStyle} px-5 py-3 rounded-2xl shadow-sm text-right min-w-[140px]`}>
             <p className="text-[10px] text-[#94A3B8] font-bold uppercase tracking-widest leading-none mb-1">{t("bukuSakit.report.idLaporan")}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
          ...
          <div className="p-8 md:p-10 flex flex-col gap-10 bg-white dark:bg-[#1E293B]">
            ...
            <div className="flex flex-col gap-4">
               <h3 className="text-[11px] font-black text-[#94A3B8] uppercase tracking-[0.2em] flex items-center gap-2">
                 <Box size={14} className="text-[#0D9488]"/> {t("bukuSakit.report.informasiAset")}
               </h3>
               <div className={`p-6 rounded-[24px] shadow-sm ${innerBoxStyle}`}>
                  <p className="text-xl font-bold text-[#0F172A] dark:text-[#F8FAFC]">{dt(report.assets?.name)}</p>
                  <div className="flex items-center gap-3 text-sm text-[#475569] dark:text-[#94A3B8] mt-2 font-medium">
                     <span className="bg-[#CCFBF1] dark:bg-[#115E59]/40 text-[#0D9488] dark:text-[#37BAAE] px-3 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wider">{dt(report.assets?.type)}</span>
                     {report.assets?.id && (
                     ...
                     <span className="uppercase tracking-widest text-[12px] font-bold text-[#475569] dark:text-[#94A3B8]">{dt(report.assets?.locations?.name)}</span>
                  </div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="flex flex-col gap-2">
                <span className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-widest">{t("bukuSakit.common.pelapor")}</span>
                <div className="flex items-center gap-3 text-[#0F172A] dark:text-[#F8FAFC] font-bold">
                  ...
                  <span className="text-[16px]">{dt(report.reporter_name)}</span>
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <span className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-widest">{t("bukuSakit.report.waktu")}</span>
                <div className="flex items-center gap-3 text-[#0F172A] dark:text-[#F8FAFC] font-bold">
                  ...
                  <span className="text-[16px]">{new Date(report.created_at).toLocaleDateString(dateLocale)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <span className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-widest">{t("bukuSakit.report.kronologiMasalah")}</span>
              <div className={`p-6 rounded-[24px] leading-relaxed shadow-sm ${innerBoxStyle}`}>
                <p className="text-[15px] text-[#475569] dark:text-[#F8FAFC]">
                  {dt(report.description) || t("bukuSakit.report.tidakAdaDeskripsi")}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-8 md:p-10 bg-white dark:bg-[#1E293B]">
            <h3 className="text-[11px] font-black text-[#94A3B8] uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
              <ImageIcon size={14} className="text-[#0D9488]"/> {t("bukuSakit.report.fotoBuktiKerusakan")}
            </h3>
            <div 
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                 <span className="bg-white/20 backdrop-blur-md text-white px-5 py-2.5 rounded-full text-xs font-bold border border-white/30 shadow-xl">{t("bukuSakit.report.resolusiPenuh")}</span>
              </div>
            </div>
            <div className="mt-6 flex items-start gap-3 p-4 bg-[#F8FAFC] dark:bg-[#0F172A] rounded-2xl border border-gray-100 dark:border-white/5">
                ...
                <p className="text-[11px] text-[#94A3B8] font-medium leading-relaxed italic">
                   {t("bukuSakit.report.gunakanLampiran")}
                </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 md:p-10 border-t border-gray-100 dark:border-[#334155] bg-white dark:bg-[#1E293B] flex justify-end">
            {existingWorkOrder ? (
              ...
              <Eye size={20} /> {t("bukuSakit.report.lihatWorkOrder")}
              </Link>
            ) : (
              ...
              {t("bukuSakit.report.terbitkanWorkOrder")} <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
              </Link>
            )}
        </div>
      </div>
      ...
      {isLightboxOpen && report.image_url && (
        ...
        >
          <img 
            ...
            alt={t("bukuSakit.report.resolusiPenuh")} 
          />
