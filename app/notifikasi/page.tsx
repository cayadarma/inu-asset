...
export default function NotificationPage() {
  ...
  // Ambil data laporan kerusakan terbaru
  const fetchNotifications = async () => {
    setIsLoading(true);
    // Kita ambil data laporan dan join (gabung) dengan data aset untuk ambil nama & lokasi_id
      ...
      <div className="flex flex-col gap-3">
        {isLoading ? (
          ...
          notifications.map((notif) => (
            <Link 
              key={notif.id}
              // Navigasi otomatis ke detail Buku Sakit aset tersebut
              href={`/buku-sakit/${notif.assets.location_id}/${notif.asset_id}`}
            ...
            >
              <div className="flex items-start gap-5">
                {/* Ikon sesuai urgensi */}
                <div className={`p-3 rounded-xl flex-shrink-0 ${notif.urgency === 'Berat (Mati Total)' ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-blue-500'}`}>
                  {notif.urgency === 'Berat (Mati Total)' ? <AlertTriangle size={24} /> : <Box size={24} />}
                </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-center gap-4 mt-1 text-[11px] text-[#94A3B8] font-medium italic">
