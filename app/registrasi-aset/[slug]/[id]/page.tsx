...
export default function AssetDetailPage({ params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  // --- FUNGSI LIGHTBOX (PENYEBAB ERROR TADI) ---
    ...
    <div className="flex flex-col gap-6 pb-10 font-poppins text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
          ...
          <div className="flex gap-3">
            <button onClick={() => setIsEditModalOpen(true)} className="flex-1 flex items-center justify-center gap-2 bg-[#0D9488] text-white py-3.5 rounded-xl font-bold text-sm shadow-md hover:bg-teal-700">
               <Edit3 size={18} /> Edit
            </button>
               ...
               {activeTab === 'kerusakan' ? (
                 damageHistory.length > 0 ? damageHistory.map((report, i) => (
                    <div key={i} className="flex justify-between items-center p-6 border-b border-gray-50 dark:border-[#334155] last:border-0 hover:bg-gray-50 dark:hover:bg-[#0F172A]/50 transition-all group">
                       ...
                       <Link href={`/buku-sakit/${slug}/${id}`} className="px-5 py-2 bg-[#96BEFF] text-[#0932B6] rounded-lg font-bold text-[12px]">Detail</Link>
                    </div>
                 )) : <p className="p-10 text-center text-secondary italic">Belum ada riwayat.</p>
               ) : (
                 <p className="p-10 text-center text-secondary italic">Riwayat pemeliharaan pencegahan akan tampil di sini.</p>
               )}
            </div>
          </div>
        </div>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Informasi Aset">
        <form onSubmit={handleUpdate} className="grid grid-cols-1 lg:grid-cols-3 gap-10 text-left">
          ...
          <div className="lg:col-span-1 flex flex-col gap-5">
             ...
             <button type="button" onClick={() => editFileInputRef.current?.click()} className="w-fit px-4 py-2 bg-[#F1F5F9] dark:bg-[#334155] border border-[#AFBDD2] rounded-lg text-[11px] font-bold text-[#475569] dark:text-[#F8FAFC]">Pilih File Baru</button>
             <div className="flex flex-col gap-3 mt-auto">
                <button type="submit" className="w-full bg-[#0D9488] text-white py-3.5 rounded-xl font-bold text-sm shadow-md">Simpan Perubahan</button>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Konfirmasi Hapus">
        <div className="flex flex-col items-center text-center gap-6 py-4">
           ...
           <p className="dark:text-white">Yakin hapus permanen <span className="font-bold text-red-600">{asset.name}</span>?</p>
           <div className="flex gap-4 w-full">
              ...
              <button onClick={handleDelete} className="flex-1 py-3 bg-[#EF4444] text-white rounded-xl font-bold">Ya, Hapus</button>
           </div>
        </div>
      </Modal>
      ...
      {isLightboxOpen && (
        <div className="fixed inset-0 z-[300] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in" onClick={() => setIsLightboxOpen(false)}>
