  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#0F172A] dark:text-[#F8FAFC]">Daftar Aset — {locationName}</h1>
          <p className="text-[#475569] dark:text-[#94A3B8] text-sm">Menampilkan semua aset yang terdaftar pada lokasi {locationName}</p>
        </div>
      ...
      <div className="bg-white dark:bg-[#1E293B] rounded-xl border border-gray-200 dark:border-[#334155] shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#F8FAFC] dark:bg-[#0F172A] dark:bg-[#0F172A] dark:bg-[#0F172A] border-b border-gray-200 dark:border-[#334155] text-[#475569] dark:text-[#94A3B8] text-sm font-bold">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-[#334155] dark:divide-[#334155]">
              {assets.map((asset) => (
                <tr key={asset.id} className="hover:bg-gray-50 dark:hover:bg-[#334155]/50 dark:hover:bg-[#334155]/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-[#475569] dark:text-[#94A3B8]">{asset.spec}</td>
                  <td className="px-6 py-4 text-sm text-[#475569] dark:text-[#94A3B8]">{asset.age}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Informasi Utama Aset">
        <form className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 flex flex-col gap-5">
             ...
             <div className="flex flex-col gap-2">
                ...
                <div className="relative">
                  <select className="w-full appearance-none px-4 py-3 border border-gray-200 dark:border-[#334155] rounded-xl bg-white dark:bg-[#1E293B] text-sm outline-none focus:border-primary cursor-pointer">
          ...
          <div className="lg:col-span-1 flex flex-col gap-5">
             ...
             <div className="w-full aspect-square bg-[#D6DEE6] rounded-xl flex flex-col items-center justify-center gap-2 border border-gray-200 dark:border-[#334155]">
...
function FormInput({ label, placeholder, type = "text", disabled = false }: any) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <input type={type} disabled={disabled} placeholder={placeholder} className={`w-full px-4 py-3 border border-gray-200 dark:border-[#334155] rounded-xl text-sm outline-none focus:border-primary ${disabled ? 'bg-[#F8FAFC] dark:bg-[#0F172A] dark:bg-[#0F172A] dark:bg-[#0F172A] cursor-not-allowed' : 'bg-white dark:bg-[#1E293B]'}`} />
    </div>
  );
}