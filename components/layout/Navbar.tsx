        ...
        return (
          <React.Fragment key={index}>
            <div className={`${isHiddenOnMobile ? "hidden md:flex" : "flex"} items-center gap-1 md:gap-2`}>
              {isLast ? (
                <span className="text-[#0F172A] dark:text-[#F8FAFC] font-bold truncate max-w-[120px] md:max-w-none">{label}</span>
              ) : (
                <Link href={finalHref} className="text-[#64748B] dark:text-[#94A3B8] font-medium hover:text-primary whitespace-nowrap">{label}</Link>
              )}
  ...
  return (
    <header className="h-[72px] bg-white dark:bg-[#1E293B] border-b border-gray-100 dark:border-[#334155] px-4 md:px-8 flex justify-between items-center sticky top-0 z-40 transition-all font-poppins">
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <Suspense fallback={<span className="text-xs text-gray-400 italic">Memuat...</span>}><Breadcrumbs /></Suspense>
      </div>
